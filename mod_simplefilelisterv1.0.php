...
$sfl_dirlocation = $params->get( 'sfl_dirlocation', '.'.DS.'images' );
...
if ( substr( $sfl_userlocation , strlen($sfl_userlocation) - 1) !== DS ) {
  $sfl_userlocation .= DS;
}
...
if ($sfl_useusernameddir == 1) {
	...
	if ($usr_id > 0 && strlen($sfl_userlocation) > 0) {	
		// Set user path, it already has the DS at the end, don't add after usr_name.
...
if(!class_exists('SFLAjaxServlet')) JLoader::register('SFLAjaxServlet' , dirname(__FILE__).DS.'helper.php');
//Security check
...
if (strpos($sfl_dirlocation, "../") !== false) $sfl_dirlocation = $params->get( 'sfl_dirlocation', '.'.DS.'images' );
