...
class ModSimpleFileListerHelperv10
{
	...
	function getDirContents($params, $sfl_dirlocation, $sfl_basepath, $sfl_maxfiles, $sfl_userlocation) {
		...
		$browsedir = "";
		$filelist = "";
		$sfl_dirlocationdefault = $params->get( 'sfl_dirlocation', '.'.DS.'images' );
		...
		if ($sfl_allowdelete === "1") {
			...
			if ($sfl_allowdeletereg === "1" && !$user->guest)
				$show_delete = "1";
			if ($sfl_allowdeleteedt === "1" && $user->authorize('com_content', 'edit', 'content', 'all'))
				$show_delete = "1";
		}
...
//echo "sfl_dirlocation=$sfl_dirlocation<br/>";
		...
		if (strlen($sfl_dirlocation) == 0 || (strlen($sfl_dirlocation) > 0 && strlen($sfl_userlocation) > 0)) {
			...
			if ( substr( $sfl_dirlocation , strlen($sfl_dirlocation) - 1) !== DS )
				$sfl_dirlocation .= DS;
			
			if (strlen($sfl_basepathusr) > 0 && $usr_name !== "") {
				if ( substr( $sfl_basepathusr , strlen($sfl_basepathusr) - 1) !== DS )
...
//echo "sfl_setbasepath=$sfl_setbasepath<br/>";
		...
		if (strlen($startdir) > 0 && str_replace(DS, "", $startdir) !== str_replace(DS, "", $sfl_dirlocation)) {
			// We have browsed!
			
			$browsedir = substr($sfl_dirlocation, strlen($startdir));
			// Remove any leading slash
			$browsedir = str_replace(DS, "/", substr($browsedir, 1));
			// Remove any trialing slash
			if ( substr( $browsedir , strlen($browsedir) - 1) === DS )
				$browsedir = substr( $browsedir, 0, strlen($browsedir) - 1);
			// Make sure we are working with front-slash only
			$browsedir = str_replace(DS, "/", $browsedir);
			...
			foreach ($pathcol as $dirval) {
				...
				if ($icntdir < count($pathcol)) {
					...
					$sfl_goupdir = "<a class=\"sfl_btnBrowseDir\" rel=\"".$startdir.$dirvals."\" href=\"javascript: void(0);\">".JText::_('UP DIR')."</a>";
				} else {
...
//$results .= "idx_startat=$idx_startat | idx_endat=$idx_endat";
			...
			if ($sfl_next === '1' && $idx_startat > 0)
				$fil_list[] = "<input type=\"hidden\" id=\"sflPrevVal\" value=\"".$idx_startat."\" /><a id=\"sfl_btnPrev\" href=\"javascript:void(0)\">".JText::_('PREV BTN')."</a>";
			else
			...
			while (false !== ($lfile = readdir($bib))) {
				...
				if (is_dir($sfl_dirlocation.DS.$lfile)) {
...
//print_r($file_list);
						...
						if ($idx > $idx_startat && $idx <= $idx_endat) {
							...
							if (($sfl_imgthumbs === '1' || $sfl_imgthumbs === '2') && !$fdir) {
								...
								if ((filesize($sfl_dirlocation.DS.$lfile['name']) <= $sfl_disablegdthreshold) || ($sfl_disablegdthreshold == 0)) {
									//Check image
									if ($img = @getimagesize($sfl_dirlocation.DS.$lfile['name'])) {
										// Show thumbnail
							...
							if ($show_delete === '1' && !$fdir)
								$tmpfile .= " <a class=\"sfl_btnDelete\" rel=\"".$sfl_dirlocation.DS.$lfile['name']."**".$lfile['name']."\" href=\"javascript:void(0)\"><img class=\"sfldel\" src=\"".JURI::root().$sfl_basepath."images/delete.png\" /></a>";
			...
			if ($sfl_next === '1' && $idx >= $idx_endat)
				$fil_list[] = "<input type=\"hidden\" id=\"sflNextVal\" value=\"".$idx_endat."\" /><a id=\"sfl_btnNext\" href=\"javascript:void(0)\">".JText::_('NEXT BTN')."</a>";
			...
			if(is_array($fil_list)) {
				$liste = "<div class=\"sfl_item\" style=\"left: ".$sfl_listleft."px\">" . join("</div><div class=\"sfl_item\" style=\"left: ".$sfl_listleft."px\">", $fil_list) . "</div>";
			} else {
				$liste = "<div class=\"sfl_item\" style=\"left: ".$sfl_listleft."px\">" . JText::_('NO FILES FOUND') . " " . $sfl_dirlocation . "</div>";
			}
			...
			if ($sfl_showdir == 1) {
				$results .=  "<b>" . JText::_('FILES IN DIR') . " (" . $sfl_breadcrumb . "):</b>";
			} elseif ($sfl_showdir == 0) {
				$results .=  "<b>".JText::_('FILES IN DIR').$sfl_currentdir.":</b><br />";
				// If no breadcrumb we must have "home" and "up" buttons
				if (strlen($browsedir) > 0)
					$results .= "<a class=\"sfl_btnBrowseDir\" rel=\"".$startdir."\" href=\"javascript: void(0);\">".JText::_('GO HOME')."</a>&nbsp;&nbsp;".$sfl_goupdir."<br />";
			}
			$results .= $sort_arrows."<div>" . $liste . "</div>";
		} else {
			$results .=  "<b>" . JText::_('ERROR READ') ." (Dir: ". $sfl_dirlocation . ")</b>";
		}
	...
	function deleteFile($params, $fileName) {
		...
		if ($show_delete === "1") {
			...
			if ($sfl_movedeleted === "1") {
				...
				if (!file_exists($sfl_movedeletedpath)) {
					...
					if (mkdir($sfl_movedeletedpath, 0777, true)) {
						...
						return JText::_('DELETE MSG2').$sfl_movedeletedpath;
			...
			if (!unlink($absoluteFilePath)) {
				return JText::_('DELETE MSG6')." ($absoluteFilePath)!";
				//return "Failed to delete file ($absoluteFilePath)!";
			} else {
				return "$fileName ".JText::_('DELETE MSG7')."!$msg_backup";
			}
			
		} else {
			return JText::_('DELETE MSG8');
